import { Link } from "react-router-dom";
import { WA_INTENT_KEY, waUrl } from "../lib/whatsapp";

export function cn(...parts) {
  return parts.filter(Boolean).join(" ");
}

export function Wrap({ className = "", children, as: Tag = "div", ...props }) {
  return (
    <Tag className={cn("relative z-[4] mx-auto w-[min(1200px,calc(100%-clamp(20px,4vw,48px)*2))]", className)} {...props}>
      {children}
    </Tag>
  );
}

export function Brand({ to = "/", className = "", onClick }) {
  const inner = (
    <img
      src="/img/logo-liberaai.png"
      alt="LiberaAI Engenharia"
      className="brand-mark select-none"
      draggable="false"
    />
  );

  const cls = cn("flex items-center no-underline cursor-pointer", className);

  if (to.startsWith("/#") || to.startsWith("#")) {
    return (
      <a className={cls} href={to.replace(/^\/#/, "#")} aria-label="LiberaAI início" onClick={onClick}>
        {inner}
      </a>
    );
  }

  return (
    <Link className={cls} to={to} aria-label="LiberaAI início" onClick={onClick}>
      {inner}
    </Link>
  );
}

export function WaIcon({ size = 16 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.435 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z" />
    </svg>
  );
}

export function WaLink({ text, className = "", children, direct = false, to = "/whatsapp/?source=LP_HOME", onClick, ...props }) {
  if (direct) {
    return (
      <a href={waUrl(text)} target="_blank" rel="noopener noreferrer" className={className} onClick={onClick} {...props}>
        {children}
      </a>
    );
  }

  return (
    <Link
      to={to}
      className={className}
      onClick={(ev) => {
        try {
          sessionStorage.setItem(WA_INTENT_KEY, text || "");
        } catch {
          /* ignore */
        }
        onClick?.(ev);
      }}
      {...props}
    >
      {children}
    </Link>
  );
}

export function Box({ as: Tag = "article", className = "", off = false, children, ...props }) {
  return (
    <Tag className={cn("box", off && "box-off", className)} {...props}>
      {children}
    </Tag>
  );
}

export function BoxNum({ sm = false, children, className = "" }) {
  return <span className={cn("box-num", sm && "box-num-sm", className)}>{children}</span>;
}

export function Eyebrow({ children, className = "" }) {
  return (
    <p className={cn("mono inline-flex items-center gap-[11px] text-accent before:block before:h-px before:w-[26px] before:bg-accent/70", className)}>
      {children}
    </p>
  );
}

export function PhotoStage({ src, tone = "wash", position = "center", className = "", eager = false }) {
  return (
    <div className={cn("photo-stage", `photo-stage-${tone}`, className)} aria-hidden="true">
      <div className="photo-stage-shift">
        <img src={src} alt="" decoding="async" loading={eager ? "eager" : "lazy"} style={{ objectPosition: position }} />
      </div>
      <div className="photo-veil" />
      <span className="photo-hud photo-hud-tl" />
      <span className="photo-hud photo-hud-tr" />
      <span className="photo-hud photo-hud-bl" />
      <span className="photo-hud photo-hud-br" />
    </div>
  );
}

export function SectionHead({ eyebrow, title, lede }) {
  return (
    <div className="max-w-[36.25rem]">
      <Eyebrow className="reveal">{eyebrow}</Eyebrow>
      <h2 className="display reveal mt-3.5 text-[clamp(2.5rem,1.7rem+3.2vw,4.1rem)]">{title}</h2>
      {lede ? <p className="reveal mt-4 max-w-[36.25rem] text-[17px] leading-[1.6] text-white">{lede}</p> : null}
    </div>
  );
}

export function MarkOk() {
  return (
    <span className="grid size-[22px] shrink-0 place-items-center rounded-full border border-accent-mid/35 bg-accent-mid/15 text-accent shadow-[0_0_10px_rgba(45,212,191,0.2)]" aria-hidden="true">
      <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.8">
        <path d="M2 6.2 4.8 9 10 3.5" />
      </svg>
    </span>
  );
}

export function MarkX() {
  return (
    <span className="grid size-[22px] shrink-0 place-items-center rounded-full border border-[#9a7b6c]/25 bg-[#9a7b6c]/12 text-[#9a7b6c]" aria-hidden="true">
      <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.8">
        <path d="M2 2l8 8M10 2 2 10" />
      </svg>
    </span>
  );
}
